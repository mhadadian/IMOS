...
{
	fprintf(stderr, "[MOSJAVA] This is a modified version of minimap. Modifier : MOSJAVA\n");
...
//		fprintf(fp_help, "    -v INT       verbose level [%d]\n", mm_verbose);
	...
	while ((mi = mm_idx_reader_read(idx_rdr, n_threads)) != 0) {
		...
		// MOS: Add Socket Receiving 
		if (strcmp(argv[o.ind+1],"minimos")==0){
			
			//MOS: Creating Socket
			
			int socket_desc , client_sock , c_sock , read_size;
			...
			if (socket_desc == -1)
			{
				fprintf(stderr, "[MOSJAVA] Could not create socket\n");
				return -1;
			}
			fprintf(stderr, "[MOSJAVA] Socket created\n");
			...
			if( bind(socket_desc,(struct sockaddr *)&server , sizeof(server)) < 0)
			{
				...
				fprintf(stderr, "[MOSJAVA] bind failed. Error\n");
				return 1;
			}
			fprintf(stderr, "[MOSJAVA] bind done\n");
			//MOS: End Creating Socket
			...
			while(1){
				...
				fprintf(stderr, "[MOSJAVA] Comunicating with IMOS Worker...\n");
				c_sock = sizeof(struct sockaddr_in);

				//accept connection from an incoming client
				client_sock = accept(socket_desc, (struct sockaddr *)&client, (socklen_t*)&c_sock);
				if (client_sock < 0)
				{
					fprintf(stderr, "[MOSJAVA] Comunicatation failed\n");
					return 1;
				}
				fprintf(stderr, "[MOSJAVA] Comunicatation accepted\n");
				
				while( (read_size = read(client_sock , client_message , sizeof(client_message))) < 1 ) {}
				strcat(client_message,"\0");
    			strcpy(argv[o.ind+1],client_message);
				if (!(opt.flag & MM_F_FRAG_MODE)) {
				for (i = o.ind + 1; i < o.ind + 2; ++i)
					mm_map_file(mi, argv[i], &opt, n_threads);
				} else {
					mm_map_file_frag(mi, argc - (o.ind + 1), (const char**)&argv[o.ind + 1], &opt, n_threads);
				}
				puts("finish@@@finish@@@finish");
				fflush(stdout);
				
			}
		}
		//MOS end add socket recieving
		else{
			if (!(opt.flag & MM_F_FRAG_MODE)) {
